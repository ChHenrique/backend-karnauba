...
export function Bigcard({imageUrl, name, text}: {imageUrl: string, text: string, name: string}){
    ...
    return(
        ...
        className="h-full cursor-pointer aspect-video max-md:aspect-square flex flex-col text-neutrals-100 justify-end items-start rounded-3xl overflow-hidden relative"
        ...
        >
            ...
            <h1 className=" z-20 max-md:text-xl text-2xl font-roboto-700 font-bold">{name}</h1>
            <h1 className=" z-20 max-md:text-sm font-spline-400 font-medium text-base">{text}</h1>
            </div>
        </div>
    )
}
